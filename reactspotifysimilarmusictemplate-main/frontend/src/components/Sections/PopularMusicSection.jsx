...
const PopularMusicSection = () => {
  ...
  const popularSongs = Array.from({ length: 15 }, (_, index) => ({
    id: `popular-${index + 1}`,
    imageUrl: "/assets/exampleMusicImg.jpeg?v=1",
    title: `Popular Hit ${index + 1}`,
    artist: `Top Artist ${index + 1}`,
  ...
  const visibleSongs = popularSongs.slice(startIndex, startIndex + itemsPerPage);
  const canGoBack = startIndex > 0;
  const canGoForward = startIndex + itemsPerPage < popularSongs.length;
