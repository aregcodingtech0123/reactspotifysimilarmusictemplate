...
import { fetchTrending, fetchNew } from '../../services/songsService';

const TrendingSongsPage = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const filterNew = searchParams.get('filter') === 'new';
  const [sortBy, setSortBy] = useState(filterNew ? 'newest' : 'popular');
  ...
  useEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    (filterNew ? fetchNew(50) : fetchTrending(50)).then((res) => {
      if (cancelled) return;
      setIsLoading(false);
