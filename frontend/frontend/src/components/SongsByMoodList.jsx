...
// Semantic categories: filter by primary_genre (vector-assigned), no string matching.
const categories = ['All', 'Rock', 'Pop', 'Jazz', 'Hip Hop', 'Rap', 'Electronic', 'R&B', 'Indie', 'Metal'];

function seeMoreLink(activeCategory) {
  if (activeCategory === 'All') return '/discover';
  return `/category/${activeCategory.toLowerCase().replace(/\s+/g, '-')}`;
}
...
const SongsByMoodList = ({ isLogged, title = 'Recommended for You', songs: songsProp, loading: loadingProp }) => {
