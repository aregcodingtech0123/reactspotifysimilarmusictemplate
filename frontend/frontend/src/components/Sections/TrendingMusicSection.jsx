/**
 * TrendingMusicSection - Shows trending songs from backend API
...
const TrendingMusicSection = () => {
  ...
  useEffect(() => {
    console.log('[TrendingMusicSection] Fetching trending songs from API...');
    let cancelled = false;
    fetchTrending(15)
      .then((res) => {
        if (cancelled) return;
        console.log('[TrendingMusicSection] API Response:', { ok: res.ok, dataLength: res.ok ? res.data?.length : 0 });
        if (res.ok && res.data) {
          // Ensure data is an array
          const songsData = Array.isArray(res.data) ? res.data : [];
          console.log('[TrendingMusicSection] SONGS FROM API:', songsData);
          setSongs(songsData);
        } else {
          console.error('[TrendingMusicSection] Failed to fetch songs:', res.error);
          setSongs([]);
        }
        setLoading(false);
      })
      .catch((error) => {
        if (cancelled) return;
        console.error('[TrendingMusicSection] Fetch error:', error);
        setSongs([]);
        setLoading(false);
      });
