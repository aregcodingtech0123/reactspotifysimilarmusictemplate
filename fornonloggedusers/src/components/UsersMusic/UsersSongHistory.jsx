...
const UsersSongHistory = () => {
  ...
  // Optional: Fetch user-specific song history when username changes
  useEffect(() => {
    // Here you would typically fetch the user's song history from an API
    // For example: fetchUserSongHistory(username)
    console.log(`Fetching song history for user: ${displayUsername}`);
  }, [displayUsername]);
  ...
  const viewSongDetails = (id) => {
    navigate(`/${displayUsername}/song/${id}`);
    setOpenMenuId(null);
  };
